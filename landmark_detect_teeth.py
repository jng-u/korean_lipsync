...
#원본 이미지 위치 
featured_image_folder_path = './data/featured_img/'
#저장할 위치
feature_croped_folder_path = './data/feature_crop/'
landmark_folder_path = './data/landmark/'
...
file_list = glob.glob(featured_image_folder_path+'*', recursive=True)
for file in file_list:
    ...
    for s in inner_mouth_shape:
        s -= [x, y]
    cv2.polylines(landmark_img, [under_shape], False, (255, 255, 255), 1)
    ...
    # find canny
    mask = np.zeros((h, w), dtype=np.uint8)
    cv2.fillPoly(mask, [inner_mouth_shape], 255)
    mask = cv2.erode(mask, np.ones((5, 5), np.uint8), 3)
    inner_mouth_masked_img = cv2.bitwise_and(croped_img, croped_img, mask=mask)
    # gray_imm = cv2.cvtColor(inner_mouth_masked_img, cv2.COLOR_RGB2GRAY)
    # # clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(5, 5))
    # # gray_imm = clahe.apply(gray_imm)
    # tot_num=0
    # elems = []
    # for y in range(gray_imm.shape[0]):
    #     for x in range(gray_imm.shape[1]):
    #         if mask[y, x] != 0:
    #             elems.insert(tot_num, gray_imm[y, x])
    #             tot_num += 1
    ...
    # # gray_imm = cv2.GaussianBlur(gray_imm, (5, 5), 3)

    # ret, gray_imm = cv2.threshold(gray_imm, 150, 255, cv2.THRESH_BINARY)
    # gray_imm = cv2.erode(gray_imm, np.ones((2, 2), np.uint8), 5)
    # gray_imm = cv2.dilate(gray_imm, np.ones((2, 2), np.uint8), 5)
    # gray_imm = cv2.dilate(gray_imm, np.ones((3, 3), np.uint8), 5)
    # gray_imm = cv2.erode(gray_imm, np.ones((3, 3), np.uint8), 5)
    
    # resized_masked_img = cv2.resize(inner_mouth_masked_img, None, fx=1/ratio, fy=1/ratio)
    # resized_masked_img = cv2.cvtColor(resized_masked_img, cv2.COLOR_BGR2GRAY)
    tooth = cv2.Canny(inner_mouth_masked_img, 0, 255)
    # tooth = cv2.Canny(gray_imm, 0, 255)
        ...
        for i in range(len(lines)):
            for rho, theta in lines[i]:
                if theta > 1:
                    ...
                    cv2.line(masked_img,(x1,y1),(x2,y2),(0,0,255),2)

    # cv2.imshow('rr', resized_img)
    cv2.imshow('landmark', landmark_img)
    cv2.imshow('masked', masked_img)
    # cv2.imshow('masked', resized_masked_img)
    cv2.imshow('ed', inner_mouth_masked_img)
    cv2.waitKey(10000)
